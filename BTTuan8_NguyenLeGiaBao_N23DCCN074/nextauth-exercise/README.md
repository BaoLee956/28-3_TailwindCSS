...
Bài lab: NextAuth Token Refresh Exercise — Mục tiêu: thực hành cơ bản về cấu hình xác thực và làm mới token trong ứng dụng Next.js. Ngôn ngữ/Framework: Next.js, Node.js.
...
- `next.config.js` — Cấu hình Next.js cho project.
- `package.json` — Khai báo dependencies và scripts (ví dụ `dev`).
- `pages/_app.js` — Điểm vào của ứng dụng, bọc các provider chung.
- `pages/index.js` — Trang chủ của bài lab.
- `pages/login.js` — Trang đăng nhập / giao diện xác thực.
- `pages/api/auth/[...nextauth].js` — API route cấu hình NextAuth (xử lý đăng nhập và refresh token).
...
> **Lưu ý**: Chỉ **ROLE_ADVISOR** có thể truy cập dashboard
